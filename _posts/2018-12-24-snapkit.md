...
初始化时持有 ConstraintView，提供了对外的调用的一些函数。是对  和 部分 UIView 功能的封装。
...
**上面的所有链式操作，内部都是对同一个 ConstraintDescription 对象的操作，ConstraintDescription 描述了当前约束的所有信息**
...
internal static func makeConstraints(item: LayoutConstraintItem, closure: (_ make: ConstraintMaker) -> Void) {
    // 生成maker 对象，里面包含了 一个 LayoutConstraintItem 对象
    let maker = ConstraintMaker(item: item)

    // 执行闭包，就是执行的上面描述的代码，生成了一个 ConstraintDescription 数组，保存这所有约束的描述
    closure(maker)
